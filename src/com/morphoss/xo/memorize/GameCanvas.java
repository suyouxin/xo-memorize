...
public class GameCanvas extends LinearLayout {
	...
	public GameCanvas(final Context context, AttributeSet attrs, int defStyle) {
		...
		LayoutParams wrapParams = new LinearLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		LayoutParams matchParams = new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);

		mLeftMenu = new RelativeLayout(context);
		final LayoutInflater inflater = (LayoutInflater) getContext()
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mLeftMenu = (RelativeLayout) inflater.inflate(R.layout.leftmenu, this,
				false);
		...
		MenuAdapter sizeAdapter = new MenuAdapter(this.getContext(),
				R.layout.menu_text_img, sizeMenus);
		...
		String[] array = getResources().getStringArray(R.array.changeTheme);
		final MenuAdapter.MenuInfo themeMenus[] = {
				new MenuAdapter.MenuInfo(array[0], R.drawable.theme),
				new MenuAdapter.MenuInfo(array[1], R.drawable.theme),
				new MenuAdapter.MenuInfo(array[2], R.drawable.theme),
				new MenuAdapter.MenuInfo(array[3], R.drawable.theme),
				new MenuAdapter.MenuInfo(array[4], R.drawable.theme), };

		MenuAdapter themeAdapter = new MenuAdapter(this.getContext(),
				R.layout.menu_text_img, themeMenus);
		mThemeSpinner.setAdapter(themeAdapter);
		mThemeSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			@Override
			public void onItemSelected(AdapterView<?> parent, View view,
					int pos, long id) {
				// change theme
				Log.d("listener", "value on spinner : "
						+ parent.getItemAtPosition(pos).toString());
				Log.d("listener", "value of pos : " + pos);
				switch (pos) {
				case 1:
					theme = 2;
					setTheme();
	...
	void setGameInfo(ArrayList<GameInfo> gameInfos) {
		...
		MenuAdapter.MenuInfo[] gameMenus = new MenuAdapter.MenuInfo[gameInfos
				.size()];
		int i = 0;
		...
		MenuAdapter gameAdapter = new MenuAdapter(this.getContext(),
				R.layout.menu_text_img, gameMenus);
